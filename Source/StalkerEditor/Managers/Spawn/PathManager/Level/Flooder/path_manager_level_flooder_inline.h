...
//		graph->unpack_xz		(graph->vertex(_start_node_index),start_position.x,start_position.y);
	...
	this->max_range_sqr			= iFloor(_sqr(this->max_range)/ this->m_sqr_distance_xz + .5f);
	this->m_cell_dist				= this->graph->header().cell_size();
}
...
{
	checkSlow					(this->graph);
	return					(this->m_cell_dist);
}
...
{
	checkSlow					(this->data_storage);
	return					(
		(iteration_count >= this->max_iteration_count)	||
		(this->data_storage->get_visited_node_count() >= this->max_visited_node_count)
	);
}
