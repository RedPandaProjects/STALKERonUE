...
IC	void CLevelStraightLinePathManager::create_path	(T &vertex)
{
	...
	u32									vertex_id = this->graph->check_position_in_direction(dwNode,tPosition,m_parameters->m_dest_point);
	if (this->graph->valid_vertex_id(vertex_id))
		fDirectDistance					= tPosition.distance_to(m_parameters->m_dest_point);
	else
		fDirectDistance					= m_parameters->max_range;
	if (fDirectDistance == m_parameters->max_range)
		m_parameters->m_distance		= fCumulativeDistance + fLastDirectDistance + m_parameters->m_dest_point.distance_to(this->graph->vertex_position((*this->path)[this->path->size() - 1]));
	else
